...
import { AtlassianMcpError, cloudIdForSite, sourcesFromSearch } from './atlassian.js';
...
test('returns only linked Jira and Confluence sources at this site', () => {
  const result = { content: [{ type: 'text', text: JSON.stringify({
    results: [
      { url: `${site}/browse/DEMO-12?token=never-share`, title: 'Fictional issue' },
      { url: `${site}/wiki/spaces/DEMO/pages/12345/Guide`, title: 'Fictional guide' },
